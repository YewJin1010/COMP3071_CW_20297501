...
class A2C_Buffer(nn.Module):
    ...
    def train_env_episode(self, render=False, replay_buffer=None):
        ...
        while not done and time < MAX_EPISODE_DURATION:
            ...
            next_observation, reward, done, truncated, info = self.env.step(action.item())
...
def train_a2c_buffer(env):
    ...
    for i in range(MAX_EPISODES):
        ...
        rewards, critic_vals, action_lp_vals, total_reward, total_violations, time = agent.train_env_episode(render=False, replay_buffer=replay_buffer)
